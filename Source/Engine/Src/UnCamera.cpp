	...
	else if( ParseCommand(&Cmd,"SHOT") )
	{
		...
		if( appFSize(File)<0 )
		{
			...
			if( F )
			{
				...
				// File header.
				FH.bfType		= LittleEndian( (_WORD)('B' + 256*'M') );
				FH.bfSize		= LittleEndian( (DWORD)(sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) + 3 * SizeX * SizeY) );
				FH.bfReserved1	= 0;
				FH.bfReserved2	= 0;
				FH.bfOffBits	= LittleEndian( (DWORD)(sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER)) );
				appFwrite( &FH, sizeof(FH), 1, F );

				// Info header.
				IH.biSize			= LittleEndian( (DWORD)sizeof(BITMAPINFOHEADER) );
				IH.biWidth			= LittleEndian( SizeX );
				IH.biHeight			= LittleEndian( SizeY );
				IH.biPlanes			= LittleEndian( (_WORD)1 );
				IH.biBitCount		= LittleEndian( (_WORD)24 );
				IH.biCompression	= 0; //BI_RGB
				IH.biSizeImage		= LittleEndian( (DWORD)(SizeX * SizeY * 3) );
